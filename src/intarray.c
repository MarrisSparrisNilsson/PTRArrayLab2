...
bool getIntArray(intArray a) { // Takes a string with numbers and commas from the user and creates an intarray if the string has the correct format.
    char input[CAPACITY];
    char *endPtr;
    fflush(stdin);
    ...
    for (size_t i = 0, k = 0; input[i] != '\n'; i++, k++) { // Loops through the string.
        ...
        int intValue = strtol(input + i, &endPtr, 10); // Returns 0 if input + i is anything else than a number.
        i = i + count; 
        k = i;


        if (intValue != 0) {
            while (intValue != SENTINEL) { // Initializes the intArray with ints as long as SENTINEL value not found.
            a[j] = intValue;
             j++;
            break;
            }
...
void appendIntArray(intArray a, intArray b, intArray c) { // Appends 2 intArrays into third intArray.
    ...
    for (size_t i = 0, j = 0; a[i] != SENTINEL || b[j] != SENTINEL;) {
        ...
        // When all values from intArray a is stored in intArray c, then the function starts putting
        // values from intArray b into the correct slots of intArray c as long as SENTINEL value not found.
